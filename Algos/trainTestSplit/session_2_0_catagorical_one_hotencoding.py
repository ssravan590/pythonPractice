
import pandas as pd
from  sklearn.preprocessing import OneHotEncoder
data = {
    "car":['A','B','C','D','E','F','G'],
    "mpg" : [20,25,30,15,18,22,8],
    "horsepower" : [130,165,150,120,120,115,200],
    "weight" : [3500,3700,2800,3200,4000,3000,7000],
    "acceleration" : [12.0,11.5,14.0,13.0,10.5,15.0,9.0],
    "orgin":["USA","Europe","Japan","USA","Japan","Europe","USA"]
}

df = pd.DataFrame(data)
print("----------------------df----------------")
print(df)
print(df.columns)
print("----------------------------------------")

print("------------------ILoc----------------------")
cat_colunms = ['orgin']
cat_data = df[cat_colunms]
print(cat_colunms)
print("----------------------------------------")
#Normalization
one_hot_model = OneHotEncoder(sparse_output=False,handle_unknown='ignore')
one_hot = one_hot_model.fit_transform(cat_data)


print("One Hot data :",one_hot)


